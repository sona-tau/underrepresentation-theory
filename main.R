...
json_text <- readLines("/Users/mine-ja/Documents/GitHub/underrepresentation-theory/variables.json", encoding = "UTF-8")

json_text <- readLines("variables.json", encoding = "UTF-8", warn = FALSE)
json_text <- paste(json_text, collapse = "\n")

# Then parse
sources_data <- fromJSON(json_text)

#sources_data <- fromJSON("variables.json")

counts <- vector("list", length(sources_data))
